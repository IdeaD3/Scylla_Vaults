...
public class Save {
    ...
    public static void savePlayerInventory(UUID uuid, Inventory inventory) {
        try {
            String base64 = inventoryToBase64(inventory);
            PlayerData.get().set("data." + uuid + ".inventory", base64);
            PlayerData.get().set("data." + uuid + ".name", Bukkit.getOfflinePlayer(uuid).getName().toLowerCase());
            utils.reloadConfig();
            return;
        } catch (Exception e) {
            e.printStackTrace();
